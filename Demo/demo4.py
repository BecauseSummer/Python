...
def getPackages(deviceName):
    while True:
        ...
        # 获取monkey进程
        orderName2 = (
            'adb -s %s shell "touch /sdcard/PackageName.txt"' % deviceName
        )  # 生成PackageName.txt文件
        excuOrder(orderName2)
        orderName3 = (
            'adb -s %s shell "pm list packages >/sdcard/PackageName.txt"' % deviceName
        )  # 将monkey进程信息放入PackageName.txt文件
        excuOrder(orderName3)
        time.sleep(2)
        orderName4 = (
            "adb -s %s pull /sdcard/PackageName.txt ." % deviceName
        )  # 将PackageName.txt文件到处到当前文件夹
...
def getMonkeyPID(deviceName):
    ...
    # 获取monkey进程
    orderName2 = (
        'adb -s %s shell "touch /sdcard/MonkeyPID.txt"' % deviceName
    )  # 生成MonkeyPID.txt文件
    excuOrder(orderName2)
    orderName3 = (
        'adb -s %s shell "ps|grep monkey >/sdcard/MonkeyPID.txt"' % deviceName
    )  # 将monkey进程信息放入MonkeyPID.txt文件
    excuOrder(orderName3)
    orderName4 = (
        "adb -s %s pull /sdcard/MonkeyPID.txt ." % deviceName
    )  # 将MonkeyPID.txt文件到处到当前文件夹
    excuOrder(orderName4)
...
class RunMonkeyThread(Thread):
    ...
    def run(self):
        ...
        if self.DeviceName == "" or self.DeviceName == "No Devices!":
            ...
            self.logNameST.SetLabel("")
            self.MonkeyRunState.SetLabel(
                "Not running: MonkeyTime must be a positive integer!"
            )
            ...
            self.logNameST.SetLabel("")
            self.MonkeyRunState.SetLabel(
                "Not running: MonkeyCount must be a positive integer!"
            )
            ...
            self.orderName1 = (
                'adb -s %s shell "monkey -p %s --throttle %s --ignore-crashes --monitor-native-crashes \
            ...
            for i in range(self.MonkeyPIDListLong):
                self.MonkeyRunState.SetLabel(
                    "Monkey PID: %s is running..." % self.MonkeyPIDList[i]
                )
                time.sleep(2)
        print("****************************")
            ...
            if self.MonkeyPIDListLong > 0:
                for i in range(self.MonkeyPIDListLong):
                    self.orderName5 = 'adb -s %s shell "kill -9 %s"' % (
                        self.DeviceName,
                        self.MonkeyPIDList[i],
                    )  # 杀死monkey进程
                    excuOrder(self.orderName5)
                    self.MonkeyRunState.SetLabel(
                        "Monkey PID: %s has been killed" % self.MonkeyPIDList[i]
                    )
        ...
        if self.DeviceName == "" or self.DeviceName == "No Devices!":
            ...
            if self.logo:
                ...
                for self.Logname in self.Lognames:
                    ...
                    findException(
                        "./MonkeyLog_%s/%s" % (self.LogFileName, self.Logname), "CRASH"
                    )
                    findException(
                        "./MonkeyLog_%s/%s" % (self.LogFileName, self.Logname),
                        "Exception",
                    )

                    self.orderName5 = (
                        "adb -s %s pull /data/anr/traces.txt ./MonkeyLog_%s/traces.txt"
...
class EventProgressThread(Thread):
    ...
    def run(self):
        ...
        self.MonkeyRunEventPercent.SetLabel(
            "The Event Percent is %s." % self.Gaugecount
        )
...
class InsertFrame(wx.Frame):
    def __init__(self, parent, id):
        ...
        DeviceDiaplay = wx.ComboBox(panel, -1, "", size=(268, -1), choices=getDevices())
        # 应用包名
        PackageLabel = wx.StaticText(panel, -1, "Package name:")
        global packageText
        packageText = wx.TextCtrl(panel, -1, "com.iapppay.pay.v4", size=(268, -1))
        packageText.SetInsertionPoint(0)

        # monkey事件之间的间隔时间（ms）
        MTLabel = wx.StaticText(panel, -1, "Monkey time:")
        global MTText
        MTText = wx.TextCtrl(panel, -1, "360", size=(268, -1))

        # monkey事件总次数
        MCLabel = wx.StaticText(panel, -1, "Monkey count:")
        global MCText
        MCText = wx.TextCtrl(panel, -1, "1000", size=(268, -1))
        ...
        MonkeyPctMotionSlider = wx.Slider(
            panel,
            -1,
            0,
            0,
        ...
        MonkeyPctTrackBallSlider = wx.Slider(
            panel,
            -1,
            0,
            0,
        ...
        MonkeyPctMajorNavSlider = wx.Slider(
            panel,
            -1,
            0,
            0,
        ...
        MonkeyPctSysKeysSlider = wx.Slider(
            panel,
            -1,
            0,
            0,
        ...
        MonkeyPctAppSwitchSlider = wx.Slider(
            panel,
            -1,
            0,
            0,
        ...
        MonkeyPctAnyEventSlider = wx.Slider(
            panel,
            -1,
            0,
            0,
        ...
        MonkeyAllEventGauge = wx.Gauge(
            panel, -1, 100, pos=(10, 10), size=(268, -1), style=wx.GA_HORIZONTAL
        )
        ...
        sizerSon1.AddMany(
            [
                ...
                MonkeyRunStateName,
                MonkeyRunStateText,
                MonkeyEventPercentName,
                MonkeyRunEventPercentText,
                MonkeyPctTouchLabel,
                MonkeyPctTouchSlider,
                MonkeyPctMotionLabel,
                MonkeyPctMotionSlider,
                MonkeyPctTrackBallLabel,
                MonkeyPctTrackBallSlider,
                MonkeyPctNavLabel,
                MonkeyPctNavSlider,
                MonkeyPctMajorNavLabel,
                MonkeyPctMajorNavSlider,
                MonkeyPctSysKeysLabel,
                MonkeyPctSysKeysSlider,
                MonkeyPctAppSwitchLabel,
                MonkeyPctAppSwitchSlider,
                MonkeyPctAnyEventLabel,
                MonkeyPctAnyEventSlider,
            ]
        )
